...
// FIXME: move to HttpContext class 解析请求行
...
bool parseRequest(Buffer* buf, HttpContext* context, Timestamp receiveTime)
{
  ...
  while (hasMore)
  {
    if (context->expectRequestLine()) // 处于解析请求行状态
    {
      ...
      if (crlf)
      {
        ...
        if (ok)
        {
          ...
          buf->retrieveUntil(crlf + 2); //将请求行从buff中取回 包括/r/n
          context->receiveRequestLine(); // 将状态改为 kExpextHeaders
        }
        else
        {
          hasMore = false;
    ...
    else if (context->expectHeaders()) // 解析header
    {
      ...
      if (crlf)
      {
        const char* colon = std::find(buf->peek(), crlf, ':'); // 冒号所在的位置
        if (colon != crlf)
        {
        ...
        else
        {
          ...
          context->receiveHeaders(); // 将状态该为requestAll
...
void HttpServer::onConnection(const TcpConnectionPtr& conn)
{
  if (conn->connected())
  {
    conn->setContext(HttpContext()); // 这里由于boost::any库的作用 可以接受任意类型的
...
{
  HttpContext* context = boost::any_cast<HttpContext>(conn->getMutableContext()); // 可以改变any
