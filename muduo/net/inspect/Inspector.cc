      ...
      processInspector_(new ProcessInspector)
{
  ...
  // 使用的时候一般在主线程中开一个监控线程 来监控服务器的状态
  // 这样做是为了防止竞态问题
  // 如果直接调用start (当前线程不是loop所属的io线程 是主线程) 那么可能当前构造函数还没有返回
  // 可能构造函数没有完全返回 对象还没有完全构造好
...
// eg:proc cmd cmdcallback helpmsg
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  else
  {
    ...
    else if (result.size() == 1) // 只有mudule 没有cmd错误 
    {
    ...
    else
    {
      ...
      if (commListI != commands_.end())
      {
        ...
        if (it != commList.end())
        {
          ArgList args(result.begin()+2, result.end()); // 传递回调函数给参数列表
          if (it->second)
          {
            resp->setStatusCode(HttpResponse::k200Ok);
            resp->setStatusMessage("OK");
            ...
            resp->setBody(cb(req.method(), args)); // 将回调函数返回的字符串传递给setBody
            ok = true;
          }
        }
      }
    ...
    if (!ok)
    {
      resp->setStatusCode(HttpResponse::k404NotFound); // 404 no found 
