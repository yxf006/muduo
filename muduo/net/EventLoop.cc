 ...
 void EventLoop::loop()
{
  ...
  while (!quit_)
  {
    ...
    pollReturnTime_ = poller_->poll(kPollTimeMs, &activeChannels_); //  返回poll关注fd的可读写事件
    ...
    for (ChannelList::iterator it = activeChannels_.begin(); // 遍历活动通道使用提前注册的回调函数 处理这些可读写事件
    ...
    doPendingFunctors();    // 为了让IO线程也能执行一些计算任务 合理利用CPU
  }
...
// 只有当前IO线程的事件回调中调用queueInLooop才不需要唤醒
  ...
  {
    wakeup(); // 唤醒别的线程 往eventfd缓冲区中写入8个大小的字段
  }
}
...
{
  return timerQueue_->addTimer(cb, time, 0.0); // 0.0不是间隔触发器
}
...
// 唤醒别的线程函数 向eventfd缓冲区写入8个字节 实现线程间的通信
...
{
  uint64_t one = 1;
  ssize_t n = sockets::read(wakeupFd_, &one, sizeof one); // 读走数据 eventfd附近缓存的8个字节
  if (n != sizeof one)
  {
...
// 不是简单地在临界区一次调用Functor 而是把回调列表swap到functors中
// 这样一方面减少了临界区的长度
// 同时 意味不会阻塞其他线程的queueLoop() 因为这个时候锁已经释放了 调用queueLoop玩队列中添加任务不会阻塞到锁释放
// 另一方面也避免了死锁 因为Functor可能再次调用queueInLooop()进行加锁 导致队列被2次加锁
// 由于doPendingFunctors()调用的Functor可能再次调用queueInLoop(cb)玩队列中添加任务
...
void EventLoop::doPendingFunctors()
{
  ...
  {
  MutexLockGuard lock(mutex_);
  functors.swap(pendingFunctors_); // 将pendFunctors_都添加到functors中 让它来执行 临时保存到了别的地方
  }
